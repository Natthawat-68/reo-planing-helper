"""Full data export for frontend compatibility."""
...
def get_full_data():
    """Return orgs, users, projects, audit for current user (matches frontend DB shape)."""
    err = require_login()
    if err:
        return err
    ...
    # ทุกคนเห็นโครงการทั้งหมดเหมือนกัน (Admin และ Manager เห็นครบทุกหน่วยงาน)
    ...
    org_list = [{"id": o.id, "name": o.name, "active": o.active, "pin": o.pin} for o in orgs]
    user_list = [{"id": u.id, "username": u.username, "password": u.password, "role": u.role, "orgId": u.org_id, "active": u.active} for u in users]
    project_list = []
    for p in projects:
