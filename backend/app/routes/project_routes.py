"""Project CRUD routes. Maintainer: นายกฤษฎาพงษ์ ทิณพัฒน์"""
...
def _project_to_json(p: Project) -> dict:
    images = [{"id": i.id, "name": i.name, "dataUrl": i.data_url} for i in p.images]
    return {
        "id": p.id,
        "orgId": p.org_id,
        "title": p.title,
...
def _safe_strip(val):
    """ปลอดภัยเมื่อรับ number หรือ null จาก JSON"""
    if val is None:
        return ""
...
def list_projects():
    ...
    q = Project.query
    # ทุกคนเห็นโครงการทั้งหมด (list เหมือน getFullData)
    if org_id:
...
def list_all_projects():
    """All projects (for manager view all, admin sees all)."""
    err = require_login()
    if err:
        return err
...
def get_project(project_id: str):
    ...
    # ทุกคนดูได้ทุกโครงการ (แก้ไข/ลบตรวจสอบใน PUT/DELETE)
...
def create_project():
    ...
    year_val = int(year) if year else 2568
    p = Project(
        id=project_id,
        org_id=org_id,
        title=title,
...
def update_project(project_id: str):
    ...
    t = _safe_strip(data.get("title") or p.title)
    if not t:
        return jsonify(message="กรุณาระบุชื่อโครงการ"), 400
    ...
    if "images" in data:
        ...
        images_val = [{"id": i.id, "name": i.name, "dataUrl": i.data_url} for i in (p.images or [])]
    if len(images_val) < 3:
        return jsonify(message="กรุณาอัปโหลดรูปกิจกรรมอย่างน้อย 3 รูป"), 400
    if len(images_val) > 4:
        return jsonify(message="รูปภาพต้องไม่เกิน 4 รูป"), 400
