...
# โฟลเดอร์ frontend (อยู่ระดับเดียวกับ backend)
...
def create_app(config_class: type[Config] = Config) -> Flask:
    ...
    db.init_app(app)
    # อนุญาต localhost ทุกพอร์ต (dev) — ไม่ต้องเพิ่มพอร์ตใหม่เมื่อเปลี่ยนเครื่องมือ
    ...
    # เสิร์ฟ Frontend (SPA) — ไม่ต้องใช้ Node.js
    @app.get("/")
    def index():
...
def register_blueprints(app: Flask) -> None:
    ...
    app.register_blueprint(pin_bp, url_prefix="/api/orgs")  # ต้องลงก่อน org_bp เพื่อให้ route /<id>/pin ถูก match ก่อน
