...
function initLogin() {
  ...
  try {
    ...
    if (last && select.querySelector(`option[value="${last}"]`)) select.value = last;
  } catch (_) {}
}
...
document.getElementById('loginBtn')?.addEventListener('click', async () => {
  ...
  try {
    ...
    showSuccess('เข้าสู่ระบบสำเร็จ', orgId === 'admin' ? 'ยินดีต้อนรับ' : `หน่วยงาน: ${(DB.orgs || []).find(o => o.id === orgId)?.name || orgId}`);
    initApp();
  } catch (e) {
